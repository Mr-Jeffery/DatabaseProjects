...
--select district, bus_line_name, count(bus_station_name) as number_of_stations from bus_stations join bus_line_details on bus_station_name=name group by district, bus_line_name
...
select bus_line_name, count(bus_station_name) as number_of_stations
...
select name, start_time, end_time, first_opening, count(line_details.station_name), intro  from lines join line_details on lines.name=line_details.line_name where name='1号线' group by name, start_time, end_time, first_opening, intro;
